...
contract EZCakeToken is IERC20('Ez Cake Token', 'BAKE') {
    ...
    function _transfer(
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (cakeDividendEnabled) {
                uint256 CakeTokens = contractTokenBalance.div(totalFees).mul(cakeiDividendRewardsFee);
    ...
    function swapAndSendCakeDividends(uint256 tokens) private {
        ...
        transferDividends(cakeDividendToken, address(cakeDividendTracker), cakeDividendTracker, cakeDividendTracker);
    }
...
contract cakeDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EzCake_CAKE_Dividend_Tracker", "EzCake_CAKE_Dividend_Tracker", 0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "EzCake_CAKE_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "EzCake_CAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EzCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "EzCake_CAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EzCake_CAKE_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FLOJRDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("EzCake_Dividend_Tracker", "EzCake_Dividend_Tracker", 0x338A09a17a7DA2E5c5a6B22344f3a49904224C79) {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "EzCake_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "EzCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EzCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "EzCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EzCake_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
